...
bool CHudAmmo::Draw(float flTime)
{
	...
	if (gHUD.hud_crosshair->value != 0) // draw if hud_crosshair is 1
	{
		...
		iHeight = 2;
		iWidth = -4 * scale;
		x = (ScreenWidth / 2) - cross_size + 2.01 + g_vLag[0];
		y = (ScreenHeight / 2) + g_vLag[1];
		(*gEngfuncs.pfnFillRGBA)(x, y, iWidth, iHeight, r, g, b, (1 - m_flCrosshairColor) * 255);

		// right bar
		iHeight = 2;
		iWidth = 4 * scale;
		x = (ScreenWidth / 2) + cross_size + g_vLag[0];
		y = (ScreenHeight / 2) + g_vLag[1];
		(*gEngfuncs.pfnFillRGBA)(x, y, iWidth, iHeight, r, g, b, (1 - m_flCrosshairColor) * 255);
		...
		iHeight = -4 * scale;
		iWidth = 2;
		x = (ScreenWidth / 2) + g_vLag[0];
		y = (ScreenHeight / 2) - cross_size + 1 + g_vLag[1];
		(*gEngfuncs.pfnFillRGBA)(x, y, iWidth, iHeight, r, g, b, (1 - m_flCrosshairColor) * 255);
		...
		iWidth = 2;
		x = (ScreenWidth / 2) + g_vLag[0];
		y = (ScreenHeight / 2) + cross_size + g_vLag[1];
		(*gEngfuncs.pfnFillRGBA)(x, y, iWidth, iHeight, r, g, b, (1-m_flCrosshairColor) * 255);
	}
	return true;
}
