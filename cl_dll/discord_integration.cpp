...
public:
	DiscordState()
		: cur_state(game_state::NOT_PLAYING), gamemode(), player_count(0), player_limit(0), seconds_total(0), start_timestamp(0), match_is_on(false), dirty(true)
	{
		update_presence();
	};
...
protected:
	void update_presence()
	{
		...
		if (cur_state != game_state::NOT_PLAYING)
		{
			...
			// Get the server name.
			if (gViewPort->m_szServerName[0] != NULL)
